<div>
    ...
    @if ($imporTerbuka)
        <x-modal :judul="__('hr.impor_karyawan_judul')" tutup="$set('imporTerbuka', false)">
            <div class="space-y-4 p-5">
                @if (! $imporBerjalan)
                    <div class="rounded-lg bg-blue-50 p-3 text-sm text-blue-900">
                        <p class="font-medium">{{ __('hr.petunjuk_impor_karyawan') }}</p>
                        <ul class="mt-1 list-disc pl-5 text-xs text-blue-800 space-y-1">
                            <li>{{ __('hr.ket_impor_karyawan_update') }}</li>
                            <li>{{ __('hr.ket_impor_karyawan_relasi') }}</li>
                            <li>{{ __('hr.ket_impor_karyawan_akun') }}</li>
                            <li>{{ __('hr.ket_impor_karyawan_kolom') }}</li>
                        </ul>
                ...
                @if ($hasilImpor)
                    <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm">
                        <p class="font-medium text-emerald-900">
                            {{ __('hr.hasil_impor_karyawan', ['baru' => $hasilImpor['baru'], 'diperbarui' => $hasilImpor['diperbarui']]) }}
                        </p>
                        @if (! empty($hasilImpor['catatan']))
                            <p class="mt-2 font-medium text-amber-900">
                                {{ __('hr.peringatan_impor', ['jumlah' => count($hasilImpor['catatan'])]) }}
                            </p>
                            <ul class="mt-1 max-h-32 space-y-0.5 overflow-y-auto text-xs text-amber-800">
                                @foreach ($hasilImpor['catatan'] as $catatan)
                                    <li>• {{ $catatan }}</li>
                                @endforeach
                            </ul>
                        @endif
                        @if (! empty($hasilImpor['dilewati']))
                            <p class="mt-2 font-medium text-rose-900">
                                {{ __('hr.baris_dilewati', ['jumlah' => count($hasilImpor['dilewati'])]) }}
                            </p>
                            <ul class="mt-1 max-h-32 space-y-0.5 overflow-y-auto text-xs text-rose-800">
