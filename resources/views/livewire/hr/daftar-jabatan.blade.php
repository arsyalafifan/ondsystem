<div>
    ...
    @if ($imporTerbuka)
        <x-modal :judul="__('hr.impor_jabatan_judul')" tutup="$set('imporTerbuka', false)">
            <div class="space-y-4 p-5">
                @if (! $imporBerjalan)
                    <div class="rounded-lg bg-blue-50 p-3 text-sm text-blue-900">
                        <p class="font-medium">{{ __('hr.petunjuk_impor_jabatan') }}</p>
                        <ul class="mt-1 list-disc pl-5 text-xs text-blue-800 space-y-1">
                            <li>{{ __('hr.ket_impor_jabatan_update') }}</li>
                            <li>{{ __('hr.ket_impor_jabatan_kolom') }}</li>
                        </ul>
                ...
                @if ($hasilImpor)
                    <div class="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm">
                        <p class="font-medium text-emerald-900">
                            {{ __('hr.hasil_impor_jabatan', ['baru' => $hasilImpor['baru'], 'diperbarui' => $hasilImpor['diperbarui']]) }}
                        </p>
