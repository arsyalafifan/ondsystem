<div>
    <x-judul-halaman :judul="__('statistik.judul_dus_terjual')" :keterangan="__('statistik.ket_dus_terjual')" />
    ...
    <x-kartu>
        ...
        <div class="grid grid-cols-1 gap-3 p-4 sm:grid-cols-3">
            <div class="rounded-xl border border-gray-200 bg-white p-4">
                <p class="text-xs text-gray-500">{{ __('statistik.total_dus_terjual') }}</p>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-right font-medium">{{ __('statistik.dus_terantar') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($this->perDriver as $i => $d)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-right tabular-nums font-semibold text-emerald-700">@angka($d['total_dus']) {{ __('umum.satuan_dus') }}</td>
                            <td class="px-4 py-2 text-right tabular-nums text-gray-600">@angka($d['total_pesanan'])</td>
                            <td class="px-4 py-2 text-right tabular-nums text-gray-600">@angka($d['total_toko'])</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-kosong ikon="trophy" :judul="__('statistik.kosong_dus_terjual')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @script
    <script>
        const chart = window.pasangChartInsentif('chart-dus-terjual', @js($this->dataChart));

        if (chart) {
            $wire.on('dus-terjual-diperbarui', (payload) => {
