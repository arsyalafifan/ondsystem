<div>
    <x-judul-halaman :judul="__('insentif.judul_sales')" :keterangan="__('insentif.ket_sales')" />
    ...
    <x-kartu>
        <div class="flex flex-wrap items-end gap-3 border-b border-gray-200 p-4">
            ...
            @elseif ($mode === 'rentang')
                <div>
                    <label class="block text-xs font-medium text-gray-600">{{ __('insentif.dari_tanggal') }}</label>
                    <input type="date" wire:model.live="dariTanggal"
                           class="mt-1 block rounded-lg border-gray-400 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 shadow-sm transition-all focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-500/20">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600">{{ __('insentif.sampai_tanggal') }}</label>
        ...
        <div class="overflow-x-auto border-t border-gray-200">
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
                    <tr>
                        <th class="w-16 px-4 py-2 font-medium">{{ __('insentif.peringkat') }}</th>
                        <th class="px-4 py-2 font-medium">{{ __('insentif.nama_sales') }}</th>
                        <th class="px-4 py-2 text-right font-medium">{{ __('insentif.dus_terantar') }}</th>
                        <th class="px-4 py-2 text-right font-medium">{{ __('insentif.jumlah_pesanan') }}</th>
                        <th class="px-4 py-2 text-right font-medium">{{ __('insentif.jumlah_toko') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-kosong ikon="trophy" :judul="__('insentif.kosong')" />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @script
    <script>
        const chart = window.pasangChartInsentif('chart-insentif', @js($this->dataChart));

        if (chart) {
            $wire.on('insentif-diperbarui', (payload) => {
